...
export function EstudianteCuentaPage() {
  const { user } = useAuth()
  ...
  const handlePasswordChange = async (event) => {
    ...
    const email =
      profileData?.estudiante?.correo_institucional ||
      profileData?.profile?.email ||
      user?.email ||
      ''

    const { error: updateError } = await updateStudentPassword({
      email,
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <PageHeader
          title="Mi cuenta"
          description="Consulta el estado de tu cuenta institucional y cambia tu contrasena."
        />
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Mi cuenta"
        description="Administra tu cuenta institucional y refuerza la seguridad de tu acceso."
      />
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-4">
            ...
            <InfoItem
              ...
              value={<AccountBadge created={profileData.estudiante?.cuenta_creada} />}
            />
          </div>
        </div>
